...
const ThriftCard = ({ item, index = 0 }) => {
  ...
  const handleChatClick = async (e) => {
    ...
    try {
      ...
      // Prefer your api client if present; fallback to fetch
      if (api?.startChat) {
  ...
  const handleLike = async (e) => {
    ...
    // Optimistic UI: update local store first
    const prev = isLiked;
  ...
  return (
    ...
    >
      <Link to={`/thrift/${itemId || ""}`} className="block">
        {/* Image */}
      ...
      <div className="px-4 pb-4">
        <button
          onClick={handleChatClick}
          className="w-full bg-gradient-to-r from-purple-600 to-blue-600 text-white py-2 px-4 rounded-lg hover:from-purple-700 hover:to-blue-700 transition-colors font-medium flex items-center justify-center space-x-2 text-sm"
        >
          <MessageCircle className="h-4 w-4" />
